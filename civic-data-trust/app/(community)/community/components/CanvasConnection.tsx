...
const CanvasConnection: React.FC<CanvasConnectionProps> = React.memo(({ 
  connection, 
  sourceNode, 
  targetNode 
}) => {
  ...
  const styling = useMemo(() => {
    ...
    return { strokeColor, isDotted };
  }, [connection.type, connectionLineStyle]);
  
  return (
    <Arrow
      points={connectionPath}
      stroke={isHovered ? '#EF4444' : styling.strokeColor} // Red on hover for delete indication
      strokeWidth={isHovered ? 3.5 : 2.5} // Thicker on hover
      fill={isHovered ? '#EF4444' : styling.strokeColor}
      pointerLength={10}
      pointerWidth={8}
      tension={0.4}
      bezier={true}
      dash={styling.isDotted ? [8, 6] : undefined}
      lineCap="round"
      lineJoin="round"
