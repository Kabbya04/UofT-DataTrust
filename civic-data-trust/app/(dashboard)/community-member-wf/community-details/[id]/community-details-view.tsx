...
export default function CommunityDetailsView({ id }: { id: string }) {
  ...
  const community = communities.find(c => c.id.toString() === id) || {
    ...
    community_category: { id: "1", name: "Healthcare" },
    coverImage: undefined,
    admins: [] // 添加缺失的 admins 属性
  };
        ...
        <div className="w-full h-48 bg-muted rounded-lg mb-6 relative overflow-hidden">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between">
            <div className="flex items-end gap-4">
              <Avatar className="w-20 h-20 border-4 border-background">
                {community.admins && community.admins.length > 0 ? (
                  <>
                    <AvatarImage src={community.admins[0].name.charAt(0)} alt={community.admins[0].name} />
                    <AvatarFallback className="bg-background text-foreground text-lg font-semibold">
                      {community.admins[0].name.substring(0, 2).toUpperCase()}
                    </AvatarFallback>
                  </>
                ) : (
                  <AvatarFallback className="bg-background text-foreground text-lg font-semibold">
            ...
            <div className="flex items-center gap-2 pb-2">
              ...
              <Button variant={community.isJoined ? "outline" : "default"} size="sm" onClick={handleJoinToggle}>
