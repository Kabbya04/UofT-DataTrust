        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2 text-sm">
              ...
              {/* Author Link - note this will still point to the unified user profile page, which is correct */}
...
export default function PopularPage() {
  ...
  const { getUserIdFromName } = useUser();

  const handleVote = (postId: number, type: 'up' | 'down') => {
    setPosts(prev =>
      prev.map(p =>
        p.id === postId
          ? { ...p, upvotes: type === 'up' ? p.upvotes + 1 : p.upvotes, downvotes: type === 'down' ? p.downvotes + 1 : p.downvotes }
          : p
      )
    );
  };
